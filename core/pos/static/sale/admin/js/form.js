...
var sale = {
    ...
    openModal: function (select, productCode) {

        var selectedValue = select.value;
        var product = this.detail.products.find((p) => p.code === productCode);

        if (product) {
            product.price_current = product[selectedValue];
        }
    ...
    updatePrice: function (input, productCode) {
        var newPrice = parseFloat(input.value.replace("$", ""));
        // Busca el producto en tu arreglo de productos por el código
        var product = this.detail.products.find((p) => p.code === productCode);

        if (product) {
            // Actualiza el precio del producto
            product.price_current = newPrice;
        }
        this.listProducts();
    },
    calculateInvoice: function () {
        ...
        this.detail.subtotal_0 = this.detail.products
            .filter((value) => !value.with_tax)
            .reduce((a, b) => a + (b.total || 0), 0);
        this.detail.subtotal_12 = this.detail.products
            .filter((value) => value.with_tax)
            .reduce((a, b) => a + (b.total || 0), 0);
        this.detail.total_dscto = this.detail.products.reduce(
            (a, b) => a + (b.total_dscto || 0),
            0
        );
        this.detail.subtotal =
            parseFloat(this.detail.subtotal_0) + parseFloat(this.detail.subtotal_12);
        this.detail.total_iva = this.detail.products
            .filter((value) => value.with_tax)
            .reduce((a, b) => a + (b.total_iva || 0), 0);
    ...
    searchProductPrice: function (product) {
        ...
        const result = validPrices.reduce((acc, curr) => (acc && acc.quantity >= curr.quantity) ? acc : curr, null);
        return product.with_tax ? (result ? result.gross_price : 0.00) : (result ? result.net_price : 0.00);
    },
        ...
        tblProducts = $("#tblProducts").DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return (
                            '<input type="text" class="form-control" autocomplete="off" name="cant" value="' +
                            row.cant +
                            '">'
                        );
                    },
                },
                {
                    ...
                    render: function (data, type, row) {
                        return (
                            '<input type="text" class="form-control" autocomplete="off" name="dscto_unitary" value="' +
                            row.dscto +
                            '">'
                        );
                    },
                ...
                },
                {
                    ...
                    class: "text-center",
                    render: function (data, type, row) {
                        return '<input type="text" class="form-control" name="price_current" autocomplete="off" disabled value="' + row.price_current + '">'
                    },
                },
                {
                    targets: [0],
    ...
    addProduct: function (item) {
        var index = this.detail.products.findIndex(
    ...
    listAdditionalInfo: function () {
        tblAdditionalInfo = $("#tblAdditionalInfo").DataTable({
            ...
            paginate: false,
            info: false,
            columns: [{data: "name"}, {data: "name"}, {data: "value"}],
            columnDefs: [
                {
                    targets: [-2],
                    class: "text-center",
                    render: function (data, type, row) {
                        return (
                            '<input type="text" class="form-control" placeholder="Ingrese un nombre" autocomplete="off" name="additional_info_name" value="' +
                            row.name +
                            '">'
                        );
                    },
                },
                {
                    ...
                    render: function (data, type, row) {
                        return (
                            '<input type="text" class="form-control" placeholder="Ingrese un valor" autocomplete="off" name="additional_info_value" value="' +
                            row.value +
                            '">'
                        );
                    },
...
document.addEventListener("DOMContentLoaded", function (e) {
    fvClient = FormValidation.formValidation(
        document.getElementById("frmClient"),
        {
            locale: "es_ES",
                    ...
                    validators: {
                        ...
                        remote: {
                            ...
                            data: function () {
                                return {
                                    parameter:
                                    fvClient.form.querySelector('[name="mobile"]').value,
                                    pattern: "mobile",
                                    action: "validate_client",
                                };
                            },
                    ...
                    validators: {
                        ...
                        remote: {
                            ...
                            data: function () {
                                return {
                                    parameter:
                                    fvClient.form.querySelector('[name="email"]').value,
                                    pattern: "email",
                                    action: "validate_client",
                                };
                            },
        ...
        .on("core.validator.validated", function (e) {
            if (!e.result.valid) {
                const messages = [].slice.call(
                    fvClient.form.querySelectorAll(
    ...
    function validateChange() {
        ...
        if (select_payment_type.val() === "efectivo") {
            if (cash < sale.detail.total) {
                ...
                return {
                    valid: false,
                    message: "El efectivo debe ser mayor o igual al total a pagar",
                };
            }
        }
    ...
    fvSale = FormValidation.formValidation(document.getElementById("frmForm"), {
        ...
        .on("core.form.valid", function () {
            ...
            if (select_receipt.val() === "1") {
                if (
                    customer.identification_type.id === "07" &&
                    sale.detail.total >= 50.0
                ) {
                    message_error(
                        "No se puede facturar este monto con un cliente consumidor final"
                    );
                    return false;
                }
                if (
                    customer.identification_type.id === "07" &&
                    select_payment_type.val() === "credito"
                ) {
                    message_error(
                        "No se puede facturar a crédito con un cliente consumidor final"
                    );
                    return false;
                }
            }
            ...
            params.append(
                "additional_info",
                JSON.stringify(
                    sale.detail.additional_info.filter(
                        (value) =>
                            !$.isEmptyObject(value.name) && !$.isEmptyObject(value.value)
                    )
                )
            );
            var args = {
...
$(function () {
    ...
    $("#tblProducts tbody")
        ...
        .on("change", 'input[name="cant"]', function () {
            ...
            var product = sale.detail.products[tr.row];
            var node = tblProducts.row(tr.row).node();
            product.cant = parseInt($(this).val());
            product.price_current = sale.searchProductPrice(product);
            sale.calculateInvoice();
            $('td:eq(5)', node).find('input[name="price_current"]').val(product.price_current.toFixed(2));
        ...
        .on("change", 'input[name="dscto_unitary"]', function () {
            ...
            parent
                ...
                .children()
                .html(sale.detail.products[tr.row].total_dscto.toFixed(2));
            $("td:last", tblProducts.row(tr.row).node()).html(
                "$" + sale.detail.products[tr.row].total.toFixed(2)
            );
        })
